...
(function (window, $) {
  ...
  // Shared: render a single book card (used by catalog + related books)
  ...
  // Shared: Add-to-Cart handler (delegated, works on catalog cards AND details page button)
  ...
  // Socket.io connection (shared instance for catalog + details pages)
  ...
  const CatalogPage = {
    ...
    clearAllFilters() {
      ...
      this.state = {
        ...
        search: this.state.search, // keep active text search
    ...
    fetchBooks() {
      ...
      // Reflect filters in the URL (shareable/bookmarkable, no page reload)
    ...
    // ── Real-time: reflect stock/catalog changes without a refresh ──
    ...
    bindSocketEvents() {
      ...
      // New book published or price/details changed by admin — refresh current page silently

      // (only worth a full refetch if we're on page 1 with default sort, to avoid disrupting deep pagination)
    ...
    bindEvents() {
      ...
      $('#buyNowBtn').on('click', function () {
        ...
        if (!TokenStore.isLoggedIn()) {
          ...
          setTimeout(() => (window.location.href = '/login?redirect=/checkout'), 900);
      ...
      $('#reviewForm').on('submit', function (e) {
        ...
        const editingReviewId = $('#reviewForm').data('editing-review-id');
        ...
        const url = editingReviewId
          ...
          ? `/books/${self.currentBook._id}/reviews/${editingReviewId}`

          : `/books/${self.currentBook._id}/reviews`;

        const method = editingReviewId ? 'put' : 'post';



        Api[method](url, { rating, comment })
  ...
  // Page router — detect which page we're on and initialize accordingly
